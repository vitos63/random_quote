...
class RandomQuoteView(DetailView):
    ...
    def get_object(self, queryset = None):
        return Quote.objects.random_quote()
...
class SearchView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['search_field'] = search_field
        if search_field:
            context['quotes_result']  = Quote.objects.select_related('author').prefetch_related('category').filter(quote__icontains=search_field, status='Published')
            context['authors_result'] =Author.objects.with_published_quotes().filter(name__icontains=search_field)
...
class AuthorQuotes(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['author'] = get_object_or_404(Author, slug=self.kwargs['author'])
