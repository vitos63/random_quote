...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    password2 = serializers.CharField(
        write_only=True, min_length=8, label="Подтверждение пароля"
    )
...
class CreateQuoteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if not attrs.get("author", "") and not attrs.get("new_author", ""):
            raise serializers.ValidationError(
                {
                    "author": "Вы должны выбрать уже существующего автора или написать своего"
                }
            )
        ...
        if not attrs.get("categories", "") and not attrs.get("new_categories", ""):
            raise serializers.ValidationError(
                {
                    "categories": "Вы должны выбрать из уже существующих категорий или написать свои"
                }
            )
    ...
    def create(self, validated_data):
        author = (
            self.validated_data.get("author", None)
            if self.validated_data.get("author", None)
            else self.validated_data["new_author"].title()
        )
