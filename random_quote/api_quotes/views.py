...
class SearchAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not any(serializer_data.values()):
            return Response({"detail": "Результаты не найдены."}, status=status.HTTP_404_NOT_FOUND)
...
class DeleteSavedQuotesAPIView(DestroyAPIView):
    ...
    def perform_destroy(self, instance):
        ...
        instance.delete()
        return Response({"detail": "Цитата успешно удалена."}, status=status.HTTP_204_NO_CONTENT)
